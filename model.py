...
def predict(username):
    ...
    review_df = pd.read_csv('./Data/sample_input.csv')
    sentiment_clean_df = pd.read_csv('./Data/sentiment_dataframe.csv')

    user_reco_file = open('./Model/user_recommendation_system.pkl', 'rb')
    user_reco_table = pickle.load(user_reco_file)

    sentiment_model_file = open('./Model/SentimentAnalysis_model.pkl', 'rb')
    sentiment_model = pickle.load(sentiment_model_file)

    tfidf_file = open('./Model/tfidf_vectorizer.pkl', 'rb')
